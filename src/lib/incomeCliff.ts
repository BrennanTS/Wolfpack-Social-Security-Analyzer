...
export function incomeCliff(analysis: HouseholdAnalysis): IncomeCliff | null {
  ...
  const finalIndexes = analysis.people.map((p) => analysis.finalIndexByPersonId[p.person.id]);
  ...
  const survivorIndex = 1 - firstIndex;

  const deathYear = Math.floor(finalIndexes[firstIndex] / 12);

  const beforePoint = analysis.combinedTimeline.find((p) => p.year === deathYear - 1);
  const afterPoint = analysis.combinedTimeline.find((p) => p.year === deathYear + 1);
  ...
  const survivorPerson = analysis.people[survivorIndex];
  const survivorLabel = personLabel(survivorPerson.person.name, survivorIndex);

  return { deathYear, before, after, dropPercent, survivorLabel };
}
