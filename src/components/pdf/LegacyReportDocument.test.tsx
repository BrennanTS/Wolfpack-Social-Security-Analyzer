...
import { buildMethodPairs, MethodologyAppendix } from './reportChrome';
import { LegacyReportDocument } from './LegacyReportDocument';
...
describe('LegacyReportDocument renders', () => {
  ...
  it('renders a married-household report without throwing, with more pages than a single', async () => {
    ...
    const singleBlob = await pdf(<LegacyReportDocument analysis={singleAnalysis} />).toBlob();
  ...
  it('prints the widowed report, not the single-claimant one', async () => {
    ...
    // walk over `LegacyReportDocument`'s own tree comes back empty and every
    ...
    expect(() => LegacyReportDocument({ analysis })).not.toThrow();
